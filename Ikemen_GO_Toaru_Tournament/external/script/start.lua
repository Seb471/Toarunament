...
function start.f_remapAI(ai)
	...
	for side = 1, 2 do
		...
		if start.p[side].teamMode == 0 or start.p[side].teamMode == 2 then --Single or Turns
			if (main.t_pIn[side] == side and not main.cpuSide[side] and not main.coop) or start.challenger > 0 or gamemode('training') or gamemode('trials') then
				setCom(side, 0)
			else
...
function start.f_getName(ref, hidden)
	if ref == nil then
		return ''
	end
		...
		for row = 1, motif.select_info.rows do
			for col = 1, motif.select_info.columns do
				...
				elseif t.char ~= nil and t.hidden == 0 then
					...
					if start.f_getCharData(t.char_ref).trialsdef == "" and gamemode("trials")  then
		...
		if ct <= #start.trials.trial and start.trials.draw.success == 0 then
			...
			if motif.trials_mode.textbox_visible == 'true' and start.trials.trial[ct].textbox ~= '' then
				...
				start.trials.draw.draw_textbox_text = main.f_textRender(
					...
					0,
					0,
					main.font_def[motif.trials_mode.textbox_text_font[1] .. motif.trials_mode.textbox_text_font_height],
...
function start.f_trialsChecker()
	...
	elseif player(1) and commandGetState(main.t_cmd[1], 'd') and commandGetState(main.t_cmd[1], 'w') and start.trials.draw.fade == 0 and motif.trials_mode.trialresetenabled == "true" then
		start.trials.draw.fadein = motif.trials_mode.fadein_time
		start.trials.draw.fadeout = motif.trials_mode.fadeout_time
		start.trials.draw.fade = start.trials.draw.fadein + start.trials.draw.fadeout
	...
	if  main.t_selChars[row].def ~= nil and main.t_selChars[row].trialsdef ~= "" then
		...
		for line in trialsFile:lines() do
			...
			elseif lcline:find("playerlife") then
					trial[i].p1life = tonumber(f_trimafterchar(lcline, "="))
			elseif lcline:find("dummylife") then
					trial[i].p2life = tonumber(f_trimafterchar(lcline, "="))
